...
public class ElasticsearchEntityMapper
		implements EntityConverter<ElasticsearchPersistentEntity<?>, ElasticsearchPersistentProperty, Object, Map>,
		EntityWriter<Object, Map>, EntityReader<Object, Map>, InitializingBean, EntityMapper {
	...
	@Override
	@Nullable
	public <R> R read(Class<R> type, Map source) {
		return doRead(source, ClassTypeInformation.from((Class<R>) ClassUtils.getUserClass(type)));
	}

	@Nullable
	protected <R> R doRead(Map source, TypeInformation<R> typeHint) {
		...
		if(typeHint.getType().isInterface()) {
			typeHint = typeMapper.readType(source, typeHint);
		}
	...
	protected <R> R readEntity(ElasticsearchPersistentEntity<?> entity, Map source) {
	...
	@Override
	public void write(@Nullable Object source, Map sink) {
	...
	protected void writeEntity(ElasticsearchPersistentEntity<?> entity, Object source, Map<String, Object> sink,
		...
		if (requiresTypeHint(entity.getTypeInformation(), containingStructure)) {
			typeMapper.writeType(entity.getTypeInformation(), sink);
		}
	...
	private Object writeEntity(Object value, ElasticsearchPersistentProperty property, TypeInformation<?> typeHint) {
		...
		writeEntity(mappingContext.getRequiredPersistentEntity(typeHint), value, target, property.getTypeInformation());
		return target;
	}
	...
	private boolean requiresTypeHint(TypeInformation<?> type, @Nullable TypeInformation<?> containingStructure) {

		if (containingStructure != null) {

			if (containingStructure.isCollectionLike()) {
				if (containingStructure.getActualType().equals(type)) {
					return false;
				}
			}
			if (containingStructure.isMap()) {
				if (containingStructure.getMapValueType().equals(type)) {
					return false;
				}
			}
			if (containingStructure.equals(type)) {
				return false;
			}
		}
	 ...
	 * @return
	 */
	private ElasticsearchPersistentEntity<?> computeClosestEntity(ElasticsearchPersistentEntity<?> entity, Map source) {
